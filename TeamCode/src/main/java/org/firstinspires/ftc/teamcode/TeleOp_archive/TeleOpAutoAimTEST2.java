...
@TeleOp(name = "TeleOpAutoAimTEST2", group = "Robot")
@Disabled
@Config

public class TeleOpAutoAimTEST2 extends OpMode {
    ...
    public void init() {
        ...
        telemetry.addLine("=== TeleOpAutoAimTEST2 ===");
...
//            pinpoint.setPosX((ROBOT_CENTER_Y + startPosY),DistanceUnit.INCH);
            ...
            if(bbx+bby+bbh != 0){
                ...
                //pinpoint.setPosY((double)(blackboard.get(xFromAutonomous))*-1.0, DistanceUnit.INCH);
                //pinpoint.setPosX((double)(blackboard.get(yFromAutonomous)), DistanceUnit.INCH);
                pinpoint.setHeading(Math.toDegrees(bbh), AngleUnit.DEGREES);
...
//        double yBot = pinpoint.getPosX(DistanceUnit.INCH);
        double xPP = pinpoint.getPosY(DistanceUnit.INCH) * -1.0;
        double yPP = pinpoint.getPosX(DistanceUnit.INCH);
        double xBot = xPP + 2*5.7 * Math.sin(Math.toRadians(angleBotDeg));
        double yBot = yPP - 2*5.7 * Math.cos(Math.toRadians(angleBotDeg));
...
//        turretMotor.setTargetPosition((int)(angleTurretDeg_raw * 3) + adjustAim - 6);  //-3 for the right bias
        turretMotor.setTargetPosition((int)(angleTurretDeg_raw * 3) + adjustAim );  //-3 for the right bias
